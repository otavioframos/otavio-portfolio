...
export function ditherMeadow(data: Uint8ClampedArray, width: number) {
  for (let i = 0; i < data.length; i += 4) {
    ...
    const threshold = .18 + bayer[(y % 4) * 4 + x % 4] / 16 * .7;
    const flower = r > 35 && r > g * 1.5 && r > b * 1.35;
    const color = light > threshold ? (flower ? [208, 65, 48] : [136, 142, 126]) : [22, 24, 22];
